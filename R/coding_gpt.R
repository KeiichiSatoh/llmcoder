...
#'
#' @param df A `data.frame` containing the text to be processed.
...
#' `"gpt-4.1"`. Examples include `"gpt-4o"` and `"gpt-3.5-turbo"`.
...
#' @param api_key Character string containing your OpenAI API key. Defaults to `openai_key()`.
...
#' }
#'
#' Supported base types include: `type_boolean()` (logical), `type_integer()` (integer),
#' `type_number()` (double), `type_string()` (string), and `type_enum()` (categorical).
#'
...
#' result <- coding_gpt(df, schema, which_col = "text", model = "gpt-4o")
#' }
#'
#' @export
...
coding_gpt <- function(
  ...
  stopifnot(is.character(col_vec))
